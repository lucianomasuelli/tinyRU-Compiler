...
public class Lexer {
    ...
    public Token nextToken() throws IOException {
        ...
        switch (currChar) {
            ...
            //Casos donde saltea, falta el caso de los comentarios y tabulación vertical
            case ' ', '\t', '\n', '\r' -> {scanner.advance();}

            //Casos donde se reconocen simbolos
            ...
            //Caso donde debe revisarse si es una asignación o un igual
            case '='-> {
                if (scanner.seeNextChar() == '=') {
                    token = new Token(TokenType.IGUAL, "==", scanner.getLine(), scanner.getColumn());
                    scanner.advance();
                    scanner.advance();
                } else {
                    token = new Token(TokenType.ASSIGN, "=", scanner.getLine(), scanner.getColumn());
                    scanner.advance();
                }
            }
            ...
            //Caso reconocer string
            case '"' -> {}

            //Caso reconocer un número
            ...
            default -> throw new IllegalSymbolError((char) currChar, scanner.getLine(), scanner.getColumn());
        }
        if(currChar == -1) {
            scanner.close();
        }
